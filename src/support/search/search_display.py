...
class SearchDisplay:  # Encapsulation
    ...
    def show(self, matches: list[dict[str, Any]], term: str) -> None:  # Abstraction
        self._logger.info(f"Displaying {min(len(matches), self._max_results)} of {len(matches)} matches for '{term}'")
        ...
        if len(matches) > self._max_results:
            ...
            self._logger.info(f"Truncated display: showing {self._max_results} of {len(matches)} total matches")
